...
Any cases of server-side failure trigger an error message that is appended to a list of `errors` defined as message partial:
...
A newly connected client authenicates as part of the [`subscribe`](#subscribe-message) message that is sent to the server to synchronize a specific wallet account up to the chain's current tip. This message must include a **signature** that verifies the ownership of provided credentials (payment/ stake) at the time of connecting.
...
> [!TIP]
> Our protocol recommends clients to persist the last received server-sent message `point`.

The following sequence diagram shows, how a client connects providing an array of `point`s and credentials. The client provides more than those two bits which is described in the [`subscribe`](#subscribe-message) message.
Subsequently, the server tries to find an intersection using the chain synchronization mini-protocol.

For an identified intersection, the server computes and publishes an ordered stream of events.
This event stream provides a chronological record of on-chain events and affectively state changes for the wallet to derive its current state.

![Sequence Diagram Synchronization](./images/02-Synchronization-Sequence.svg)

#### Resumability

A client may disconnect and resume the synchronization process from its last known `point`.

#### Rollback Handling

![TODO]

- strategy with client-side window stability window of k blocks
